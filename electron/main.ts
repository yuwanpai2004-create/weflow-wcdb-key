...
type ImportedDbKeyResult = {
  success: boolean
  key?: string
  dbDir?: string
  source?: 'all_keys' | 'passphrase' | 'raw'
  error?: string
}
...
const getDbDirFromKeyPayload = (payload: any): string | undefined =>
  payload && typeof payload._db_dir === 'string' ? payload._db_dir : undefined

const looksLikeAllKeysPayload = (payload: any): boolean => {
  if (!payload || typeof payload !== 'object') return false
  if (typeof payload._db_dir === 'string') return true
  return Object.values(payload).some((entry: any) => Boolean(normalizeHex64(entry?.enc_key)))
}

const importDbKeyFile = async (filePath: string): Promise<ImportedDbKeyResult> => {
  const raw = await readFile(filePath, 'utf8')
  const asRawHex = normalizeHex64(raw)
  if (asRawHex) return { success: true, key: asRawHex, source: 'raw' }

  let payload: any
  try {
    payload = JSON.parse(raw)
  } catch {
    return { success: false, error: '无法识别密钥文件：请选择 wechat-passphrase.json，或只包含 64 位十六进制 passphrase 的文本文件' }
  }

  const passphrase = normalizeHex64(payload.passphrase || payload.decryptKey || payload.key)
  if (passphrase) {
    return {
      success: true,
      key: passphrase,
      dbDir: getDbDirFromKeyPayload(payload),
      source: 'passphrase'
    }
  }

  if (looksLikeAllKeysPayload(payload)) {
    return {
      success: false,
      dbDir: getDbDirFromKeyPayload(payload),
      source: 'all_keys',
      error: '这是 all_keys.json，里面的 enc_key 是单个数据库的派生密钥，导入后会导致错误码 -3。请导入 wcdb-key-tool 生成的 wechat-passphrase.json，或粘贴 64 位 passphrase。'
    }
  }

  return { success: false, error: '密钥文件中没有找到可用的 64 位 passphrase' }
}
...
function registerIpcHandlers() {
  ...
  ipcMain.handle('key:importDbKeyFile', async (_, filePath: string) => {
    return importDbKeyFile(filePath)
  })
